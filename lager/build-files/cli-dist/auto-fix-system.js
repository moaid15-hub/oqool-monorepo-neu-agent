...
export class AutoFixSystem {
    ...
    async fix(options) {
        ...
        for (const stageName of stagesToRun) {
            ...
            try {
                const stageResult = await this.runStage(stageName, currentCode, file, stage.action, interactive && !autoApply);
                // تحديث الإحصائيات
     ...
     * تشغيل مرحلة واحدة
     */
    async runStage(stageName, code, file, action, interactive) {
     ...
     */
    async handleInteractiveSecurityFixes(code, issues) {
     ...
     * عرض نتائج المرحلة
     */
    displayStageResults(stage, result) {
 ...
 */
export function createAutoFixSystem(workingDir) {
    return new AutoFixSystem(workingDir);
}
//# sourceMappingURL=auto-fix-system.js.map