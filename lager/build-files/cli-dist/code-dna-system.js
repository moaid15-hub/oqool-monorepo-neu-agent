...
export class CodeDNASystem {
     ...
     * استخراج DNA الكود
     */
    async extractCodeDNA(filePath, content) {
     ...
     * استخراج الأنماط
     */
    async extractPatterns(ast, content) {
        const patterns = [];
        const visitor = {
     ...
     * حساب المقاييس
     */
    async calculateMetrics(ast, content) {
        const lines = content.split('\n');
    ...
    analyzeCodingStyle(content) {
        ...
        if (camelCaseMatches?.length === maxMatches)
            namingConvention = 'camelCase';
        else if (snakeCaseMatches?.length === maxMatches)
            namingConvention = 'snake_case';
        else if (kebabCaseMatches?.length === maxMatches)
            namingConvention = 'kebab-case';
        else if (pascalCaseMatches?.length === maxMatches)
            namingConvention = 'PascalCase';
        // تحليل التعليقات
        const inlineComments = lines.filter(line => line.includes('//')).length;
        const blockComments = (content.match(/\/\*[^*]*\*\//g) || []).length;
        let commentStyle = 'mixed';
        if (inlineComments > blockComments)
            commentStyle = 'inline';
        else if (blockComments > inlineComments)
            commentStyle = 'block';
     ...
     * تقييم الجودة
     */
    assessQuality(ast, content, metrics) {
        // قابلية القراءة
     ...
     * حساب الموثوقية
     */
    calculateReliability(ast, content) {
        let reliability = 100;
        // فحص معالجة الأخطاء
     ...
     * بناء تاريخ التطور
     */
    async buildEvolutionHistory(filePath, currentHash) {
     ...
     * توليد توقيع الكود
     */
    generateCodeSignature(hash, patterns, metrics, style, complexity, quality) {
     ...
     * مقارنة DNA ملفين
     */
    async compareCodeDNA(file1, file2) {
    ...
    calculateSimilarity(dna1, dna2) {
        ...
        factors += 0.2;
        // تشابه الأنماط
        const commonPatterns = dna1.patterns.filter(p1 => dna2.patterns.some(p2 => p2.category === p1.category)).length;
     ...
     * توليد التوصيات
     */
    generateRecommendations(dna1, dna2) {
     ...
     * حساب اختلاف الأسلوب
     */
    calculateStyleDifference(style1, style2) {
        let differences = 0;
        let totalFactors = 0;
        ...
        if (style1.braceStyle !== style2.braceStyle)
            differences++;
        totalFactors++;
        if (style1.quoteStyle !== style2.quoteStyle)
            differences++;
        totalFactors++;
        if (Math.abs(style1.maxLineLength - style2.maxLineLength) > 20)
            differences++;
        totalFactors++;
...
export function createCodeDNASystem(apiClient, workingDir) {
    return new CodeDNASystem(apiClient, workingDir);
}
//# sourceMappingURL=code-dna-system.js.map