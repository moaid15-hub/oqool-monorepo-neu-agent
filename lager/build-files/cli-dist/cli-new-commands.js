...
export function registerNewCommands(program) {
    // ========================================
    ...
    program
        ...
        try {
            ...
            await db.initializeDatabase(type, options.orm);
        }
        catch (error) {
            console.error(chalk.red(error.message));
        }
    ...
    program
        ...
        .action(async (snapshotId, outputPath) => {
        try {
            const guardian = createVersionGuardian({
                apiKey: process.env.ANTHROPIC_API_KEY || '',
                projectPath: process.cwd()
    ...
    program
        ...
        .action(async (filePath, snapshotId) => {
        try {
            const archaeology = createFileArchaeology(process.cwd());
...
//# sourceMappingURL=cli-new-commands.js.map