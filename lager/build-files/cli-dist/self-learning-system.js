...
export class SelfLearningSystem {
    ...
    // ============================================
    async learnFromProject(project) {
    ...
    // 2. Analyze Success
    // ============================================
    async analyzeSuccess(project) {
        const result = project.result;
        const score = Math.round((result.analytics.qualityScore * 0.4) +
    ...
    async savePattern(pattern) {
        ...
        const existing = this.memory.patterns.find(p => this.calculateSimilarity(p.task, pattern.task) > 0.8);
        if (existing) {
    ...
    async learnFromErrors(errors) {
        for (const error of errors) {
            const existing = this.memory.errors.find(e => e.type === error.type && e.description === error.description);
            if (existing) {
                existing.frequency++;
                if (error.solution) {
    ...
    // 6. Update Strategies
    // ============================================
    async updateStrategies(project, success) {
    ...
    async getRecommendations(task) {
        ...
        if (lessons.length > 0) {
            ...
            for (const lesson of lessons) {
                recommendations.push(`   • "${lesson.task}" (${Math.round(lesson.relevanceScore * 100)}% similar, score: ${lesson.pattern.rating}/100)`);
            }
        }
        // Get common errors to avoid
        const category = this.categorizeTask(task);
        const relevantErrors = this.memory.errors.filter(e => e.type.includes(category) || e.frequency > 5);
        ...
        if (bestStrategies.length > 0) {
            ...
            for (const strategy of bestStrategies) {
                recommendations.push(`   • ${strategy.name} (success rate: ${Math.round(strategy.successRate)}%)`);
            }
        }
    ...
    // Helper Methods
    // ============================================
    extractPatterns(project) {
        const patterns = [];
        const arch = project.architecture;
        if (arch.database)
            patterns.push(`database:${arch.database.type}`);
        if (arch.frontend)
            patterns.push(`frontend:${arch.frontend.framework}`);
        if (arch.api)
            patterns.push(`api:${arch.api.authentication}`);
        patterns.push(`components:${arch.components.length}`);
        return patterns;
    }
    identifyStrategies(project) {
        const strategies = [];
        const arch = project.architecture;
        if (arch.database)
            strategies.push('database-integration');
        if (arch.frontend && arch.api)
            strategies.push('full-stack');
        if (arch.api?.authentication)
            strategies.push('authentication');
        if (arch.components.length > 5)
            strategies.push('modular-architecture');
        return strategies;
    }
    categorizeTask(task) {
...
// Factory
// ============================================
export function createSelfLearningSystem(apiKey, memoryPath) {
