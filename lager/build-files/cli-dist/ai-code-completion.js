...
export class AICodeCompletion {
    constructor(projectRoot, client) {
        this.analyzer = createCodeAnalyzer();
    ...
    async completeCode(prompt, options = {}) {
        try {
            const { language = 'typescript', maxSuggestions = 5, useContext = true, includeImports = true, includeComments = true, style = 'detailed' } = options;
            ...
            const aiPrompt = this.buildCompletionPrompt(prompt, context, language, includeImports, includeComments, style);
...
`;
     ...
     */
    async completeFunctionFromComment(comment, functionName, options = {}) {
...
`;
     ...
     * إكمال كلاس كامل
     */
    async completeClass(className, description, options = {}) {
        const language = options.language || 'typescript';
        const prompt = `
...
`;
     ...
     */
    async suggestImprovements(code, language = 'typescript') {
        const prompt = `
...
`;
     ...
     */
    async getSmartSnippets(context, language = 'typescript', category) {
     ...
     * البحث عن snippet
     */
    async searchSnippets(query, language = 'typescript') {
        const allSnippets = await this.getSmartSnippets('', language);
        const results = allSnippets.filter(s => s.name.toLowerCase().includes(query.toLowerCase()) ||
     ...
     * استخدام snippet
     */
    async useSnippet(snippetId, variables = {}) {
     ...
     */
    async recognizePatterns(code, language = 'typescript') {
...
`;
     ...
     * جمع سياق المشروع
     */
    async gatherProjectContext(language) {
        try {
...
`;
            ...
            const analyses = await Promise.all(files.slice(0, 3).map(f => this.analyzer.analyzeFile(f)));
...
`;
     ...
     * بناء prompt للإكمال
     */
    buildCompletionPrompt(userPrompt, context, language, includeImports, includeComments, style) {
...
`;
     ...
     */
    parseSuggestions(aiResponse, maxSuggestions) {
        const suggestions = [];
        ...
        for (const block of blocks.slice(0, maxSuggestions)) {
            ...
            if (codeMatch) {
                suggestions.push({
                    type: suggestionMatch?.[1]?.trim() || 'statement',
     ...
     * توليد snippets ذكية
     */
    async generateSmartSnippets(language, category) {
  ...
  try {
     ...
     */
    getContextLines(lines, currentLine, contextSize = 10) {
     ...
     */
    async findFiles(extension, limit = 10) {
        const files = [];
        const scanDir = async (dir, depth = 0) => {
            if (depth > 3 || files.length >= limit)
                return;
            try {
     ...
     */
    async learnFromUsage(prompt, acceptedSuggestion, rating) {
        // تحديث التاريخ
        const entry = this.completionHistory.find(h => h.prompt === prompt && h.suggestion === acceptedSuggestion);
        if (entry) {
            entry.accepted = true;
        }
...
// Factory Function
// ============================================
export function createAICodeCompletion(projectRoot, client) {
    return new AICodeCompletion(projectRoot, client);
}
//# sourceMappingURL=ai-code-completion.js.map