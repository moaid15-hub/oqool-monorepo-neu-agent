...
export class LearningSystem {
    constructor(workingDirectory, apiKey) {
        this.errorHistory = [];
    ...
    // 🔍 البحث عن حلول سابقة
    // ============================================
    async findSolution(error) {
        console.log(chalk.cyan('🔍 البحث عن حلول سابقة...'));
        // 1. البحث في الأخطاء المماثلة
        const similarErrors = this.errorHistory.filter(record => record.successful &&
            record.solution &&
...
ما الحل المقترح؟ أعطني حل مباشر وواضح.`
    ...
    // ✅ تسجيل نجاح حل
    // ============================================
    async recordSuccess(errorId, solution) {
        ...
        if (errorRecord) {
            ...
            if (pattern) {
                ...
                let solutionEntry = pattern.solutions.find(s => this.calculateSimilarity(s.solution, solution) > 0.8);
                if (solutionEntry) {
                    // تحديث معدل النجاح
    ...
    classifyError(error) {
        ...
        if (errorLower.includes('typeerror'))
            return 'TYPE_ERROR';
        if (errorLower.includes('referenceerror'))
