...
export class FileManager {
    ...
    async getProjectContext(maxFiles = 10) {
    ...
    async getDirectoryStructure(maxDepth = 3) {
        try {
    ...
    async buildTree(dir, prefix, maxDepth, currentDepth) {
     ...
     * @returns نجاح العملية
     */
    async applyPatches(filePath, patches) {
        try {
     ...
     * ```
     */
    extractPatchesFromResponse(response) {
        const filePatches = [];
     ...
     */
    async previewPatch(filePath, patch) {
        try {
            const fullPath = path.join(this.workingDir, filePath);
            if (!(await fs.pathExists(fullPath))) {
...
export function createFileManager(workingDir) {
