...
export class CloudLearningSync {
    ...
    // ============================================
    async downloadCommunityKnowledge(task) {
        try {
    ...
    // ============================================
    async getCommunityRecommendations(task) {
        try {
            ...
            if (stats.totalUsers > 0) {
                recommendations.push(`🌍 Community: ${stats.totalUsers} developers, ${stats.totalProjects} projects`);
            }
            if (patterns.length > 0) {
                recommendations.push('\n📚 Top community patterns:');
                for (const pattern of patterns.slice(0, 3)) {
                    recommendations.push(`   • "${pattern.task}" (${pattern.upvotes} 👍, ${pattern.downloads} downloads, score: ${pattern.rating}/100)`);
                }
            }
...
// Factory
// ============================================
export function createCloudLearningSync(options) {
...
`;
//# sourceMappingURL=cloud-learning-sync.js.map