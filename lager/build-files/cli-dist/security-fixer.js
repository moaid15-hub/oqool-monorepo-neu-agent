...
export class SecurityFixer {
     ...
     * كشف ثغرات الحقن
     */
    detectInjectionVulnerabilities(code) {
        const issues = [];
        const lines = code.split('\n');
        lines.forEach((line, index) => {
            // SQL Injection
     ...
     */
    applyDirectSecurityFixes(code, issues) {
        let fixedCode = code;
        for (const issue of issues) {
            if (!issue.fix || issue.fix === 'complex')
                continue;
            switch (issue.fix) {
                case 'remove_eval':
                ...
                case 'replace_with_textcontent':
                    fixedCode = fixedCode.replace(/(\w+)\.innerHTML\s*=\s*([^;]+);/g, '$1.textContent = $2; // Changed from innerHTML for security');
                    break;
                case 'use_strong_hash':
                    ...
                    break;
                case 'use_cipher_iv':
                    fixedCode = fixedCode.replace(/crypto\.createCipher/g, 'crypto.createCipheriv // TODO: Add IV parameter');
                    break;
                case 'use_env_var':
...
\`\`\``;
    ...
    calculateSecurityScore(issues) {
        let score = 100;
        for (const issue of issues) {
            if (issue.type === 'DangerousPattern')
                score -= 30;
            else if (issue.type === 'SQLInjection')
                score -= 25;
            else if (issue.type === 'CommandInjection')
                score -= 25;
            else if (issue.type === 'WeakCrypto')
                score -= 20;
            else if (issue.type === 'HardcodedSecret')
                score -= 20;
            else if (issue.type === 'PathTraversal')
                score -= 15;
            else
                score -= 10;
        }
...
//# sourceMappingURL=security-fixer.js.map