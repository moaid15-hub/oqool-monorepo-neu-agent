...
`;
     ...
     * توثيق رد AI
     */
    async documentResponse(prompt, response, metadata) {
     ...
     * تحديث ملف الفهرس
     */
    async updateIndex(documentation) {
        try {
    ...
    async searchDocumentation(query) {
        try {
            ...
            const allDocs = await fs.readJson(indexPath);
            const searchTerm = query.toLowerCase();
            return allDocs.filter(doc => doc.prompt.toLowerCase().includes(searchTerm) ||
    ...
    async getStatistics() {
        try {
            ...
            const allDocs = await fs.readJson(indexPath);
            const languagesUsed = new Map();
            const providersUsed = new Map();
     ...
     * تصدير كـ CSV
     */
    async exportAsCSV(docs, filePath) {
     ...
     * تصدير كـ Markdown
     */
    async exportAsMarkdown(docs, filePath) {
        ...
        for (let i = 0; i < docs.length; i++) {
            ...
            if (doc.language || doc.provider) {
                ...
                if (doc.language)
                    markdown += `- اللغة: ${doc.language}\n`;
                if (doc.provider)
                    markdown += `- المزود: ${doc.provider}\n`;
                if (doc.executionTime)
                    markdown += `- وقت التنفيذ: ${doc.executionTime}ms\n`;
                markdown += '\n';
            }
...
export function createAIDocumentation(config) {
    return new AIResponseDocumentation(config);
}
//# sourceMappingURL=ai-response-documentation.js.map