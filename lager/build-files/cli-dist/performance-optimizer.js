...
export class PerformanceOptimizer {
     ...
     */
    detectNestedLoops(ast, code) {
        const issues = [];
        const loopDepth = new Map();
        traverse(ast, {
            enter(path) {
     ...
     */
    detectExpensiveOperationsInLoops(ast, code) {
        const issues = [];
        traverse(ast, {
            ForStatement: (path) => this.checkExpensiveInLoop(path, issues),
            WhileStatement: (path) => this.checkExpensiveInLoop(path, issues),
            ForOfStatement: (path) => this.checkExpensiveInLoop(path, issues),
        });
        return issues;
    }
    /**
     ...
     */
    checkExpensiveInLoop(path, issues) {
        path.traverse({
