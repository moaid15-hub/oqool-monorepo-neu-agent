...
export class EnhancedExecutor {
     ...
     * فحص الكود (Linting)
     */
    async lintFile(filePath, autoFix = false) {
        try {
            const language = this.detectLanguage(filePath);
            if (!language) {
                return {
            ...
            try {
                ...
                if (language === 'python' && stdout) {
                    ...
                    try {
                        ...
                        issues = pylintResults.map((issue) => ({
                            file: filePath,
                            line: issue.line,
                            column: issue.column,
     ...
     */
    async formatAndLintFiles(files, autoFix = false) {
        let formatted = 0;
        let linted = 0;
        const allIssues = [];
     ...
     */
    async checkLanguageTools(language) {
        const config = this.languageConfigs.get(language);
        if (!config) {
            return {
                executor: false,
...
// تصدير instance
export function createEnhancedExecutor(workingDir) {
