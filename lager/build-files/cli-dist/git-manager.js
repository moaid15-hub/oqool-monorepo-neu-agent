...
export class GitManager {
     ...
     */
    generateBranchName(prompt, prefix = 'feature') {
     ...
     * عرض diff بشكل جميل
     */
    async displayDiff(staged = false) {
     ...
     */
    generateCommitMessage(files, prompt) {
        if (prompt) {
     ...
     */
    async autoWorkflow(files, prompt, options = {}) {
        const { autoCommit = true, autoPush = false, branchPrefix = 'feature', commitMessage } = options;
        // التحقق من git repo
        if (!(await this.isGitRepo())) {
            return {
                success: false,
...
// تصدير instance جاهز
export function createGitManager(workingDir) {
