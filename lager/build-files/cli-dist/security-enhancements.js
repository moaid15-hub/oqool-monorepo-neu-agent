...
export class SecurityEnhancements {
     ...
     */
    async scanGeneratedCode(code, fileName) {
     ...
     */
    async scanBeforeExecution(filePath) {
     ...
     * توقيع الكود رقمياً
     */
    async signCode(filePath, author) {
        try {
            const content = await fs.readFile(filePath, 'utf8');
     ...
     */
    async verifyCodeSignature(filePath) {
        try {
            const signaturePath = `${filePath}.sig`;
            if (!await fs.pathExists(signaturePath)) {
                return false;
            }
            const signature = await fs.readJson(signaturePath);
            const content = await fs.readFile(filePath, 'utf8');
     ...
     */
    async encryptSensitiveFile(filePath, key) {
        try {
            const content = await fs.readFile(filePath, 'utf8');
     ...
     * فك تشفير الملفات
     */
    async decryptSensitiveFile(encryptedPath, key) {
        try {
     ...
     * فحص إضافي للملفات القابلة للتنفيذ
     */
    async scanExecutableFile(content, filePath) {
        const issues = [];
        let score = 100;
        // فحص استخدام eval
...
export function createSecurityEnhancements(apiClient, workingDir) {
    return new SecurityEnhancements(apiClient, workingDir);
}
//# sourceMappingURL=security-enhancements.js.map