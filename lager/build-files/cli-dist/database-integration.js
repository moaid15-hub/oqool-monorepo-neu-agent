...
export class DatabaseIntegration {
     ...
     */
    async initializeDatabase(dbType, ormType) {
     ...
     */
    async generateSchemaFromDescription(description, options = {}) {
        const { includeTimestamps = true, includeSoftDelete = false, useUUID = true, generateRelations = true, ormType = 'none' } = options;
        const prompt = `
...
`;
     ...
     */
    async generateSchemaFiles(tables, ormType = 'none') {
     ...
     */
    async createMigration(name, tables) {
...
`;
     ...
     */
    async generateQueryFromNaturalLanguage(description, dbType = 'postgresql') {
        const prompt = `
...
`;
     ...
     * تحسين query موجود
     */
    async optimizeQuery(query, dbType = 'postgresql') {
        const prompt = `
...
`;
     ...
     */
    async generateSeedData(tables, rowsPerTable = 10) {
        const seedData = {};
        for (const table of tables) {
            const prompt = `
...
`;
     ...
     * إنشاء ملف seed
     */
    async createSeedFile(tableName, data) {
...
`;
     ...
     */
    async generateDatabaseConfig(dbType, ormType) {
        const prompt = `
...
`;
     ...
     */
    async generateConnectionFile(dbType, ormType) {
        const templates = {
            postgresql: `
import { Pool } from 'pg';
...
`
     ...
     */
    getDependencies(dbType, ormType) {
        const baseDeps = {
...
${table.columns.map(col => {
            ...
            if (table.primaryKey?.includes(col.name)) {
                ...
                if (col.default !== undefined)
                    colOptions.push(`default: ${JSON.stringify(col.default)}`);
...
// Factory Function
// ============================================
export function createDatabaseIntegration(projectRoot, client) {
