...
export class TestRunner {
    ...
    // ⚙️ تنفيذ الاختبارات
    // ============================================
    executeTests(framework, testFile) {
    ...
    parseTestOutput(output, framework) {
        ...
        if (framework === 'jest') {
            ...
            if (passedMatch)
                result.passed_count = parseInt(passedMatch[1]);
            if (failedMatch)
                result.failed_count = parseInt(failedMatch[1]);
            if (totalMatch)
        ...
        // Mocha
            ...
            if (passingMatch)
                result.passed_count = parseInt(passingMatch[1]);
            if (failingMatch)
                result.failed_count = parseInt(failingMatch[1]);
