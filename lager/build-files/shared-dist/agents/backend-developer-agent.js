...
export class BackendDeveloperAgent {
    ...
    // Choose tech stack
    // ============================================
    async chooseTechStack(requirements) {
    ...
    // Design API
    // ============================================
    async designAPI(requirements, techStack) {
        const prompt = `
...
Design complete CRUD operations for all resources!
    ...
    // Design database
    // ============================================
    async designDatabase(requirements, techStack) {
...
Design complete schema for all features!
    ...
    async designAuthentication(requirements) {
        const hasUsers = requirements.users || requirements.features.some((f) => f.toLowerCase().includes('user') || f.toLowerCase().includes('auth'));
        if (!hasUsers) {
            return {
                strategy: 'None',
...
5. Password reset: Email token flow
    ...
    // ============================================
    async chooseCaching(requirements) {
        const expectedLoad = requirements.expectedLoad || 'medium';
    ...
    // Plan middleware
    // ============================================
    async planMiddleware(requirements) {
        const middleware = [
        ...
        const hasAuth = requirements.features.some((f) => f.toLowerCase().includes('auth') || f.toLowerCase().includes('user'));
        if (hasAuth) {
    ...
    // ============================================
    async designErrorHandling(techStack) {
        return `
...
- Log to console (dev) + file/service (prod)
    ...
    // Design validation
    // ============================================
    async designValidation(techStack) {
        const validators = {
...
//# sourceMappingURL=backend-developer-agent.js.map