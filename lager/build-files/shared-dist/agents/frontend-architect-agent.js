...
export class FrontendArchitectAgent {
    ...
    async design(requirements) {
        ...
        const recommendations = await this.generateRecommendations(requirements, structure, routing, stateManagement);
        return {
    ...
    // ============================================
    async designComponentStructure(requirements) {
        const prompt = `
...
${requirements.pages ? `Pages:\n${requirements.pages.map((p) => `- ${p}`).join('\n')}` : ''}
...
\`\`\`
    ...
    // Plan routing
    // ============================================
    async planRouting(requirements, structure) {
    ...
    // ============================================
    async chooseStateManagement(requirements, structure) {
        const hasAuth = requirements.features.some((f) => f.toLowerCase().includes('auth') || f.toLowerCase().includes('login'));
        const hasComplexState = structure.length > 20;
        const isRealtime = requirements.features.some((f) => f.toLowerCase().includes('realtime') || f.toLowerCase().includes('chat'));
        let solution;
        let stores = [];
    ...
    // ============================================
    async planStyling(requirements) {
    ...
    // ============================================
    generateFileStructure(structure, framework) {
        const tree = {};
        // Group by directory
...
`;
    ...
    // ============================================
    async generateRecommendations(requirements, structure, routing, stateManagement) {
...
//# sourceMappingURL=frontend-architect-agent.js.map