...
dotenv.config();
import { saveConfig, logout, validateApiKey, displayAccountInfo, hasApiKey } from './auth.js';
...
program
    ...
    .option('-v, --var <key=value>', 'تعيين متغير', (value, previous = {}) => {
...
program
    ...
    try {
        const executor = createEnhancedExecutor();
        const tools = await executor.checkLanguageTools(language);
        console.log(chalk.cyan(`\n🔧 أدوات ${language}:\n`));
...
program
    ...
    try {
        ...
        if (options.interactive) {
            ...
            if (options.body)
                variables.description = options.body;
    ...
    try {
        ...
        switch (action) {
            ...
            case 'export':
                ...
                const exportPath = await docManager.exportDocumentation(format);
    ...
    try {
        ...
        switch (action) {
            case 'create':
                ...
                const members = membersInput.split(',').map((email) => email.trim());
    ...
    try {
        ...
        switch (action) {
            ...
            case 'cluster':
                ...
                const participantList = participants.split(',').map((name) => ({
                    id: name.trim(),
                    name: name.trim(),
                    type: 'ai',
                    expertise: ['general'],
...
program
    ...
    try {
        ...
        const configPath = await testGenerator.generateTestConfig(framework, options.output);
...
program
    ...
    try {
        ...
        const result = await testGenerator.runTests(framework || 'jest');
        if (result.success) {
...
program
    ...
    try {
        const wizard = createConfigWizard(process.cwd());
        const result = await wizard.quickSetup(preset, options.language);
        if (result.success) {
            console.log(chalk.green(`\n${result.message}\n`));
        }
        else {
...
program
    ...
    try {
        const wizard = createConfigWizard(process.cwd());
        const content = await wizard.exportConfig(format);
        if (options.output) {
...
program
    ...
    try {
        const tracker = createProgressTracker(process.cwd());
        await tracker.initialize();
        await tracker.updateTaskStatus(taskId, status);
...
program
    ...
    try {
        ...
        if (result.code.files.length > 0) {
            await library.saveSnippet(`god-${Date.now()}`, result.code.files[0].content, ['god-mode', ...result.architecture.tags], `God Mode: ${task}`);
        }
    }
    catch (error) {
