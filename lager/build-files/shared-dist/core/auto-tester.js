...
export class AutoTester {
    ...
    // ============================================
    async testGeneratedCode(code, filePath) {
    ...
    async checkJavaScriptSyntax(code) {
        try {
    ...
    async checkTypeScriptSyntax(code) {
        try {
    ...
    async checkPythonSyntax(code, filePath) {
    ...
    async checkJavaSyntax(code, filePath) {
        return new Promise((resolve) => {
    ...
    async getSyntaxDetails(code, filePath) {
        const ext = filePath.split('.').pop();
        const errors = [];
        try {
    ...
    // 3️⃣ Security Scan
    // ============================================
    async scanSecurity(code, filePath) {
        try {
    ...
    async getSecurityDetails(code, filePath) {
        try {
            if (!this.securityScanner) {
                return {
                    safe: true,
    ...
    async getPerformanceDetails(code, filePath) {
        const warnings = [];
        if (this.hasNestedLoops(code)) {
    ...
    printSummary(result) {
        ...
        // Syntax
        console.log(result.syntaxOk
            ? chalk.green('✅ Syntax: صحيح')
            : chalk.red('❌ Syntax: خاطئ'));
        // Tests
        console.log(result.testsPass
...
// 🏭 Factory Function
// ============================================
export function createAutoTester(config) {
