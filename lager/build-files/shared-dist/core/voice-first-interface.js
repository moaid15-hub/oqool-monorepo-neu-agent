...
export class VoiceFirstInterface {
     ...
     * معالجة الأمر الصوتي
     */
    async processVoiceCommand(text, session) {
     ...
     * تنفيذ الأمر الصوتي
     */
    async executeVoiceCommand(command, session) {
     ...
     * معالجة أمر الإنشاء
     */
    async handleCreateCommand(command, session) {
     ...
     * معالجة أمر التعديل
     */
    async handleModifyCommand(command, session) {
        const { action, parameters } = command.intent;
     ...
     * معالجة أمر التحليل
     */
    async handleAnalyzeCommand(command, session) {
        const { action, parameters } = command.intent;
        switch (action) {
            case 'code':
                if (parameters.get('files')?.length > 0) {
     ...
     * معالجة أمر التنفيذ
     */
    async handleExecuteCommand(command, session) {
        const { action, parameters } = command.intent;
        if (parameters.get('files')?.length > 0) {
            const files = parameters.get('files');
     ...
     * معالجة أمر البحث
     */
    async handleSearchCommand(command, session) {
        const { action, parameters } = command.intent;
        if (parameters.get('files')?.length > 0) {
     ...
     * معالجة أمر التنقل
     */
    async handleNavigateCommand(command, session) {
     ...
     * معالجة أمر التكوين
     */
    async handleConfigureCommand(command, session) {
     ...
     * معالجة أمر المساعدة
     */
    async handleHelpCommand(command, session) {
...
قل "خروج" لإنهاء الجلسة.`;
     ...
     * تحسين نماذج النية
     */
    async improveIntentModels(samples) {
        // حفظ عينات التدريب
     ...
     * تحسين استخراج الكيانات
     */
    async improveEntityExtraction(samples) {
...
export function createVoiceFirstInterface(apiClient, workingDir) {
    return new VoiceFirstInterface(apiClient, workingDir);
}
//# sourceMappingURL=voice-first-interface.js.map