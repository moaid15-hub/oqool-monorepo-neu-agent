...
export class OqoolAPIClient {
    ...
    apiKey;
    baseURL;
    constructor(apiKey, baseURL = 'https://oqool.net') {
        this.apiKey = apiKey;
    ...
    async sendChatMessage(messages, provider) {
        try {
    ...
    async generateCode(prompt, fileContext) {
        try {
...
export async function createClientFromConfig() {
    // Check for any available API key (DeepSeek, OpenAI, or Claude)
    const hasValidClaudeKey = process.env.ANTHROPIC_API_KEY?.startsWith('sk-ant-');
    // Only use Claude if we have a valid Claude key
    ...
    // Check for other providers (DeepSeek or OpenAI)
    if (process.env.DEEPSEEK_API_KEY || process.env.OPENAI_API_KEY) {
        console.log(chalk.cyan('🔗 استخدام UnifiedAIAdapter...\n'));
        // Return null to let the calling code use UnifiedAIAdapter
        return null;
    }
    ...
    if (!config || !config.apiKey) {
        ...
        console.log(chalk.gray('أو أضف أحد المفاتيح في ملف .env:'));
        console.log(chalk.gray('  - DEEPSEEK_API_KEY (موصى به)'));
