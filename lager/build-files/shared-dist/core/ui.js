...
export class UI {
    ...
    // رسالة ترحيب محسّنة
    showWelcome() {
        const welcomeBox = boxen(this.gradients.success('مرحباً في Oqool!') + '\n\n' +
    ...
    // عرض رد AI
    showAIResponse(response, provider) {
    ...
    formatResponse(response) {
        ...
        // تلوين العناوين
        formatted = formatted.replace(/^(#{1,6})\s+(.+)$/gm, (_, hashes, title) => chalk.cyan.bold(title));
        return formatted;
    }
    // تسمية المزود
    ...
    // عرض معلومات المشروع
    showProjectInfo(totalFiles, totalSize) {
    ...
    // عرض بنية المشروع
    showProjectStructure(structure) {
        console.log(chalk.blue('\n🌳 بنية المشروع:'));
        console.log(chalk.gray('─'.repeat(40)));
    ...
    // ============================================
    // إنشاء صندوق احترافي
    createBox(content, title, type = 'info') {
        const borderColors = {
            info: '#3b82f6',
            success: '#10b981',
            error: '#ef4444',
