...
export class TestGenerator {
    ...
    async generateTests(files, options = {}) {
        const { framework = 'jest', type = 'unit', useAI = false, generateMocks = true, includeEdgeCases = true, outputDir = path.join(this.workingDir, '__tests__'), language = 'ar' } = options;
        const result = {
            success: true,
            testsGenerated: 0,
            files: [],
        ...
        try {
            ...
            for (const file of files) {
                try {
                    const testFile = await this.generateTestFile(file, { framework, type, useAI, generateMocks, includeEdgeCases, language });
                    if (testFile) {
     ...
     */
    async generateTestFile(filePath, options) {
     ...
     */
    generateTestHeader(sourceFile, options) {
        const moduleName = path.basename(sourceFile, path.extname(sourceFile));
        const importPath = path.relative(path.join(this.workingDir, '__tests__'), sourceFile).replace(/\\/g, '/');
        let header = '';
     ...
     * توليد اختبار دالة
     */
    async generateFunctionTest(func, options) {
     ...
     * توليد حالات اختبار
     */
    async generateTestCases(func, options) {
        const testCases = [];
        // حالة عادية
     ...
     * توليد حالة عادية
     */
    generateNormalCase(func, options) {
     ...
     * توليد edge cases
     */
    generateEdgeCases(func, options) {
        const cases = [];
        // Empty input
     ...
     * توليد حالات الأخطاء
     */
    generateErrorCases(func, options) {
        const cases = [];
     ...
     */
    generateExpectedOutput(func, inputs) {
     ...
     * توليد كود الاختبار
     */
    generateTestCode(functionTest, options) {
     ...
     */
    generateTestCaseCode(functionName, testCase, options) {
     ...
     * توليد اختبار كلاس
     */
    async generateClassTest(cls, options) {
        const methods = [];
     ...
     */
    generateClassTestCode(classTest, options) {
     ...
     */
    generateMethodTestCase(methodName, testCase, options) {
        let code = `    it('${testCase.name}', async () => {\n`;
        const inputParams = Object.entries(testCase.input)
            .map(([key, value]) => JSON.stringify(value))
            .join(', ');
     ...
     */
    async generateTestConfig(framework, outputDir) {
        let config = '';
        if (framework === 'jest') {
            config = `{
  "preset": "ts-jest",
...
export default defineConfig({
  ...
  "watch-files": ["src/**/*.ts", "__tests__/**/*.ts"]
        ...
        const configPath = path.join(outputDir, framework === 'vitest' ? 'vitest.config.ts' :
 ...
 * إنشاء مولد اختبارات
 */
export function createTestGenerator(workingDir) {
