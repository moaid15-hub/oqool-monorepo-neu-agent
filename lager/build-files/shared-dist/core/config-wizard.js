...
export class ConfigWizard {
    ...
    async collectConfig(language) {
        ...
        // اسم المشروع
        config.name = await this.ask(language === 'ar' ? 'اسم المشروع:' : 'Project name:', path.basename(this.workingDir));
        // الوصف
        config.description = await this.ask(language === 'ar' ? 'وصف المشروع:' : 'Project description:', '');
        // API Key
        const needsApi = await this.askYesNo(language === 'ar'
            ? 'هل تريد استخدام Oqool AI API؟'
            : 'Do you want to use Oqool AI API?', true);
        if (needsApi) {
            config.apiKey = await this.ask(language === 'ar' ? 'API Key:' : 'API Key:', '');
            config.apiUrl = await this.ask(language === 'ar' ? 'API URL:' : 'API URL:', 'https://oqool.net');
        }
        // Git
        config.gitEnabled = await this.askYesNo(language === 'ar' ? 'تفعيل Git؟' : 'Enable Git?', true);
        // AI
        config.aiEnabled = await this.askYesNo(language === 'ar' ? 'تفعيل ميزات AI؟' : 'Enable AI features?', true);
        // Caching
        config.caching.enabled = await this.askYesNo(language === 'ar' ? 'تفعيل التخزين المؤقت؟' : 'Enable caching?', true);
        if (config.caching.enabled) {
            const ttl = await this.ask(language === 'ar' ? 'مدة الصلاحية (بالثواني):' : 'TTL (seconds):', '3600');
            config.caching.ttl = parseInt(ttl) * 1000;
        }
        // Performance
        config.performance.parallel = await this.askYesNo(language === 'ar' ? 'تفعيل المعالجة المتوازية؟' : 'Enable parallel processing?', true);
        if (config.performance.parallel) {
            const concurrency = await this.ask(language === 'ar' ? 'عدد العمليات المتزامنة:' : 'Concurrency limit:', '5');
            config.performance.concurrency = parseInt(concurrency);
        }
        config.performance.incremental = await this.askYesNo(language === 'ar' ? 'تفعيل التحليل التدريجي؟' : 'Enable incremental analysis?', true);
        // Features
        console.log(language === 'ar'
            ? '\n📦 اختر الميزات المطلوبة:'
            : '\n📦 Select features:');
        config.features.templates = await this.askYesNo(language === 'ar' ? 'القوالب (Templates)؟' : 'Templates?', true);
        config.features.hooks = await this.askYesNo(language === 'ar' ? 'Git Hooks؟' : 'Git Hooks?', true);
        config.features.review = await this.askYesNo(language === 'ar' ? 'مراجعة الكود (Code Review)؟' : 'Code Review?', true);
        config.features.docs = await this.askYesNo(language === 'ar' ? 'توليد التوثيق؟' : 'Documentation generation?', true);
        config.features.tests = await this.askYesNo(language === 'ar' ? 'توليد الاختبارات؟' : 'Test generation?', true);
        return config;
    }
    /**
     * سؤال عن نوع المشروع
     */
    async askProjectType(language) {
        console.log(language === 'ar'
            ? '\nنوع المشروع:'
            : '\nProject type:');
     ...
     * إنشاء إعدادات سريعة
     */
    async quickSetup(preset, language = 'ar') {
     ...
     * استيراد الإعدادات
     */
    async importConfig(source, format) {
        try {
            let config;
            if (format === 'json') {
 ...
 * إنشاء معالج إعدادات
 */
export function createConfigWizard(workingDir) {
