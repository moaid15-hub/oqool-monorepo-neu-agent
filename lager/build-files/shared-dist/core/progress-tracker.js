...
export class ProgressTracker {
     ...
     * إنشاء milestone
     */
    async createMilestone(name, options = {}) {
        const milestone = {
            id: this.generateId(),
            name,
            description: options.description || '',
     ...
     */
    async addTaskToMilestone(milestoneId, taskId) {
        const milestone = this.milestones.get(milestoneId);
        if (!milestone) {
            throw new Error(`Milestone ${milestoneId} not found`);
        }
    ...
    calculateStats() {
        ...
        const tasksCompletedLastWeek = allTasks.filter(t => t.completedAt && t.completedAt >= oneWeekAgo).length;
        // تقدير الإكمال
    ...
    async generateReport(period) {
        ...
        // المهام في الفترة
        const allTasks = Array.from(this.tasks.values());
        const recentTasks = allTasks.filter(t => t.updatedAt >= reportPeriod.from && t.updatedAt <= reportPeriod.to);
        // المهام المعطلة
    ...
    generateRecommendations(tasks) {
        ...
        // المهام المتأخرة
        const overdueTasks = tasks.filter(t => t.estimatedHours &&
            t.actualHours &&
...
<body>
  ...
  <div class="milestone">
    ...
    async saveData() {
        ...
        await fs.writeFile(path.join(this.dataPath, 'tracker.json'), JSON.stringify(data, null, 2), 'utf-8');
    }
    /**
     * تحميل البيانات
     */
 ...
 * إنشاء متتبع تقدم
 */
export function createProgressTracker(workingDir) {
