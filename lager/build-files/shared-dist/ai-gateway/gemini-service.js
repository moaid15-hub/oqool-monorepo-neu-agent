...
export class GeminiService {
    ...
    async chatCompletion(messages, options = {}) {
        const { model = 'gemini-2.0-flash-exp', // الأسرع: $0.10/$0.40 per 1M tokens
        maxTokens = 8192, temperature = 0.7, systemPrompt, } = options;
        try {
    ...
    async *chatCompletionStream(messages, options = {}) {
        const { model = 'gemini-2.0-flash-exp', maxTokens = 8192, temperature = 0.7, systemPrompt, } = options;
        try {
            const genAI = this.client.getGenerativeModel({
                model,
                generationConfig: {
     ...
     */
    calculateCost(inputTokens, outputTokens, model = 'gemini-2.0-flash-exp') {
        // Gemini pricing
        const pricing = {
